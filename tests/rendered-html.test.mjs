...
test("server-renders the Chinese snow fighting game for a Chinese browser", async () => {
  ...
  assert.match(html, /title="移除 AI"[^>]*>−<\/button>/);
  assert.doesNotMatch(html, />移除 AI<\/button>/);
  assert.equal(html.match(/class="roster-card__remove-ai"/g)?.length, 5);
  ...
  assert.match(html, /SAT 英语 · 大型词库/);
  assert.match(html, /托福英语 · 大型词库/);
  assert.ok(html.indexOf("托福英语 · 大型词库") < html.indexOf("SAT 英语 · 大型词库"));
  assert.match(html, /3853 词/);
  assert.match(html, /5412 词/);
  assert.match(html, /4851 词/);
  assert.match(html, /9055 词/);
  assert.match(html, /6951 词/);
  assert.match(html, /<option value="cet4" selected="">大学四级 · 大型词库/);
  assert.doesNotMatch(html, /经典情景英语/);
...
test("server-renders the complete English lobby for a non-Chinese browser", async () => {
  ...
  assert.doesNotMatch(html, />Remove AI<\/button>/);
  assert.equal(html.match(/class="roster-card__remove-ai"/g)?.length, 5);
  ...
  assert.match(html, /SAT English · Large Wordbook/);
  assert.match(html, /TOEFL English · Large Wordbook/);
  assert.ok(html.indexOf("TOEFL English · Large Wordbook") < html.indexOf("SAT English · Large Wordbook"));
  assert.match(html, /3853 words/);
  assert.match(html, /5412 words/);
  assert.match(html, /4851 words/);
  assert.match(html, /9055 words/);
  assert.match(html, /6951 words/);
  assert.match(html, /<option value="cet4" selected="">CET-4 · Large Wordbook/);
  assert.doesNotMatch(html, /Classic Situational English/);
